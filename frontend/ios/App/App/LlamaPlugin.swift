...
@objc(LlamaPlugin)
public class LlamaPlugin: CAPPlugin, AVSpeechSynthesizerDelegate, UIDocumentPickerDelegate {
    private var isLoaded = false
    private var loadedModelPath = ""
    private let speechSynthesizer = AVSpeechSynthesizer()
    private var pickModelCall: CAPPluginCall?
    ...
    @objc func speak(_ call: CAPPluginCall) {
        ...
        let utteranceId = call.getString("utteranceId") ?? ""
        
        // Configure AVAudioSession for playback to ensure sound is played even in silent mode
        do {
        ...
        let utterance = IdentifiableSpeechUtterance(string: cleanText)
    ...
    public func speechSynthesizer(_ synthesizer: AVSpeechSynthesizer, didStart utterance: AVSpeechUtterance) {
        let uttId = (utterance as? IdentifiableSpeechUtterance)?.utteranceId ?? ""
        notifyListeners("speakStatus", data: ["status": "start", "utteranceId": uttId])
    }
    
    public func speechSynthesizer(_ synthesizer: AVSpeechSynthesizer, didFinish utterance: AVSpeechUtterance) {
        let uttId = (utterance as? IdentifiableSpeechUtterance)?.utteranceId ?? ""
        notifyListeners("speakStatus", data: ["status": "done", "utteranceId": uttId])
    }
    ...
    // MARK: - Local AI Model Helper & Picker
    
    private func resolveModelPath(_ path: String) -> String {
        let fileManager = FileManager.default
        let documentsURL = fileManager.urls(for: .documentDirectory, in: .userDomainMask).first!
        
        // If it starts with "Documents/", strip it and append to documentsURL
        ...
        // If it's a relative path (doesn't start with /), assume it's in Documents
        if !path.hasPrefix("/") {
            return documentsURL.appendingPathComponent(path).path
        }
        
        // Resolve sandbox container dynamic GUID changes on relaunch
    ...
    @objc func checkModelExists(_ call: CAPPluginCall) {
        ...
        let fileManager = FileManager.default
        let exists = fileManager.fileExists(atPath: resolvedPath) && fileManager.isReadableFile(atPath: resolvedPath)
        call.resolve(["exists": exists, "resolvedPath": resolvedPath])
    }

    @objc func selectModelFile(_ call: CAPPluginCall) {
        self.pickModelCall = call
        
        DispatchQueue.main.async {
            // Use .item instead of .data to avoid greyed out files, and asCopy: false for direct copy progress
            ...
            if let rootVC = self.bridge?.viewController {
                ...
                call.reject("Could not find view controller to present document picker")
            }
        }
    }
    ...
    public func documentPicker(_ controller: UIDocumentPickerViewController, didPickDocumentsAt urls: [URL]) {
        guard let sourceURL = urls.first else {
            pickModelCall?.reject("No file selected")
            return
        }
        
        let shouldAccess = sourceURL.startAccessingSecurityScopedResource()
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            do {
                ...
                // Get source file size for copy progress
                let sourceAttributes = try fileManager.attributesOfItem(atPath: sourceURL.path)
                let totalBytes = sourceAttributes[.size] as? Int64 ?? 1
                
                // Open streams for block-by-block copying with progress updates
                guard let inputStream = InputStream(url: sourceURL) else {
                    throw NSError(domain: "LlamaPlugin", code: 1, userInfo: [NSLocalizedDescriptionKey: "Failed to open source file"])
                }
                guard let outputStream = OutputStream(url: destinationURL, append: false) else {
                    throw NSError(domain: "LlamaPlugin", code: 2, userInfo: [NSLocalizedDescriptionKey: "Failed to open destination file"])
                }
                ...
                while inputStream.hasBytesAvailable {
                    ...
                    if bytesRead < 0 {
                        throw inputStream.streamError ?? NSError(domain: "LlamaPlugin", code: 3, userInfo: [NSLocalizedDescriptionKey: "Read error"])
                    }
                    if bytesRead == 0 {
                        break
                    }
                    ...
                    if bytesWritten < 0 {
                        throw outputStream.streamError ?? NSError(domain: "LlamaPlugin", code: 4, userInfo: [NSLocalizedDescriptionKey: "Write error"])
                    }
                    
                    bytesCopied += Int64(bytesRead)
                    
                    let now = Date()
                    if now.timeIntervalSince(lastNotificationTime) > 0.3 {
                        let progress = Int((Double(bytesCopied) / Double(totalBytes)) * 100)
                        self.notifyListeners("copyProgress", data: [
                            "status": "copying",
                            "progress": progress,
                            "copied": bytesCopied,
                            "total": totalBytes
                ...
                UserDefaults.standard.set(destinationURL.path, forKey: "kalamspark_model_path")
                
                let attributes = try fileManager.attributesOfItem(atPath: destinationURL.path)
                let fileSize = attributes[.size] as? Int64 ?? 0
                
                DispatchQueue.main.async {
                    self.pickModelCall?.resolve([
                        "status": "done",
                ...
                DispatchQueue.main.async {
                    self.pickModelCall?.reject("Failed to copy file: \(error.localizedDescription)")
                }
            }
        }
    }
    ...
    public func documentPickerWasCancelled(_ controller: UIDocumentPickerViewController) {
        pickModelCall?.resolve([
            "status": "cancelled"
        ])
    }
}

class IdentifiableSpeechUtterance: AVSpeechUtterance {
