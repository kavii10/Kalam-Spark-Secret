...
// ─── Error Boundary — prevents blank screen on uncaught render errors ─────────
...
> {
  ...
  componentDidCatch(error: Error, info: React.ErrorInfo) {
    console.error('[OnboardingErrorBoundary] Caught render error:', error, info);
  }
  render() {
    if (this.state.hasError) {
      return (
        <div style={{
          ...
          background: this.props.isLight ? '#faf7f2' : '#070e20', zIndex: 100,
        }}>
          <div style={{ fontSize: 48 }}>⚠️</div>
          <p style={{ color: this.props.isLight ? '#111827' : '#fde68a', fontWeight: 600, fontSize: 18, textAlign: 'center' }}>
            Something went wrong
          </p>
          <p style={{ color: this.props.isLight ? '#6b7280' : 'rgba(211,156,59,0.6)', fontSize: 13, textAlign: 'center', maxWidth: 320 }}>
            {this.state.error || 'An unexpected error occurred during setup.'}
          </p>
          <button
            onClick={() => this.setState({ hasError: false, error: '' })}
            style={{
              marginTop: 8, padding: '12px 28px', borderRadius: 12, fontWeight: 600,
              background: 'linear-gradient(135deg, #f97316, #ea580c)',
              color: '#ffffff', border: 'none', cursor: 'pointer', fontSize: 14,
            }}
          >
            Try Again
          </button>
...
const AppContent = ({
  ...
  if (!user.onboardingComplete) {
    return (
      <>
        ...
        <OnboardingErrorBoundary isLight={isLight}>
          <Onboarding
            isLight={isLight}
  ...
  const pageTitle = {
    ...
    "/opportunities": `Kalam — Jobs & Competitions`,
  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden text-gold-100/90 select-none">
      ...
      >
        <div className="flex flex-col h-full p-4">
          ...
          <nav className="flex-1 space-y-1 overflow-y-auto no-scrollbar">
            ...
            <SidebarItem id="nav-planner" to="/planner" icon={Calendar} label="Daily Tasks" active={location.pathname === "/planner"} onClick={() => setIsSidebarOpen(false)} />
            ...
            <SidebarItem id="nav-opportunities" to="/opportunities" icon={Radio} label="Jobs & Competitions" active={location.pathname === "/opportunities"} onClick={() => setIsSidebarOpen(false)} />
      ...
      <main className={`flex-1 flex flex-col min-w-0 relative h-full overscroll-contain scroll-smooth ${
        ['/filespeaker', '/mentor'].includes(location.pathname) ? 'overflow-y-hidden' : 'overflow-y-auto'
      }`}>
        ...
        <div className="flex-1 relative">
          ...
          <div className={`page-transition ${
            ['/filespeaker', '/mentor'].includes(location.pathname) 
              ? 'h-[calc(100dvh-4rem)] p-0 lg:p-0 pb-0 overflow-hidden' 
              : 'p-5 lg:p-8 pb-28 lg:pb-10'
          }`}>
            <Routes>
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4" onClick={() => setShowSettingsModal(false)}>
          <div className="w-full max-w-sm rounded-2xl p-6 relative flex flex-col max-h-[90vh] overflow-y-auto no-scrollbar settings-modal" style={{ background: "rgba(6,3,18,0.9)", border: "1px solid rgba(255,140,66,0.22)", boxShadow: "0 20px 40px rgba(0,0,0,0.5)" }} onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex flex-col gap-3 mb-5">
              ...
              <div className="flex flex-col gap-1.5 p-3.5 rounded-lg bg-black/40 border border-gold-500/20 text-left">
                <span className="text-xs font-semibold text-gold-200">Custom Backend URL</span>
                <input 
                  type="text" 
                  value={user.settings?.customBackendUrl || ""} 
                  onChange={(e) => {
                ...
                />
                <p className="text-[9px] text-gold-500/40 mt-1 leading-relaxed">
                  Overrides default server address. Connects to your custom Render host or local machine IP (e.g. http://192.168.1.10:8000).
                </p>
              </div>
            </div>
...
export default function App() {
  ...
  // ── Background Daily Tasks Checker/Generator ──
  const taskCheckRef = React.useRef<string | null>(null);
  useEffect(() => {
    if (user && user.id && user.dream && user.onboardingComplete) {
      const todayStr = new Date().toDateString();
      const checkKey = `${user.id}_${user.dream}_${user.lastTaskResetDate}_${todayStr}`;
      if (taskCheckRef.current !== checkKey) {
        taskCheckRef.current = checkKey;
        import("./services/taskBackgroundService").then(({ checkAndGenerateDailyTasks }) => {
          checkAndGenerateDailyTasks(user, setUser).catch(err => {
            console.error('[App] Background daily task check failed:', err);
          });
        });
      }
    }
  ...
  useEffect(() => {
    ...
    return () => clearTimeout(timer);
  }, [user.id, user.xp, user.streak, user.lastLoginDate, user.lastTaskResetDate, user.currentStageIndex, user.onboardingComplete, user.settings, user.dream, user.avatar]);
  ...
  useEffect(() => {
    ...
    if (lastLogin !== today) {
       ...
       setUser(prev => ({ ...prev, streak: newStreak, lastLoginDate: today }));
    }
  }, [user.isAuthenticated, user.id, user.lastLoginDate, user.streak]);
