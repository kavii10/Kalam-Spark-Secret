...
function TaskRevisionTab({ user, onXpGain }: Props) {
  ...
  const [nextReviewDays, setNextReviewDays] = useState<number>(1);
  ...
  // Re-fetch due tasks whenever the user returns to this tab/app.
  // This is crucial so that tasks which became due overnight appear immediately.
  useEffect(() => {
    ...
    // Also listen for Capacitor native app resume (Android/iOS background→foreground)
    let removeListener: (() => void) | null = null;
    CapacitorApp.addListener('resume', () => load()).then(handle => {
      removeListener = () => handle.remove();
    }).catch(() => {}); // silently ignore if not on native
  ...
  const submitQuiz = async () => {
    ...
    const interval = await taskRevisionService.recordReview(activeRevision.id, scorePercent);
    setNextReviewDays(interval);
    onXpGain(xp);
    setShowResults(true);
    await load();
  };
  ...
  if (activeRevision) {
    ...
    if (showResults && quiz) {
      ...
      // Compute exact calendar date for the next review
      const nextReviewDate = new Date();
      nextReviewDate.setDate(nextReviewDate.getDate() + nextReviewDays);
      const nextReviewDateStr = nextReviewDate.toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" });
      // Use the real FSRS-computed interval (stored in state after submitQuiz)
      const nextIntervalText = nextReviewDays === 1
        ? 'tomorrow'
        : nextReviewDays < 7
          ...
          : nextReviewDays < 14
            ...
            : nextReviewDays < 21
              ? `in 2 weeks`
              : nextReviewDays < 60
                ? `in ${Math.round(nextReviewDays / 7)} weeks`
                : `in ~${Math.round(nextReviewDays / 30)} month${Math.round(nextReviewDays / 30) !== 1 ? 's' : ''}`;
      return (
        <div className="w-full space-y-4 fade-up">
          <div className={glassCard("text-center")} style={isLight ? { border: "1px solid rgba(211,156,59,0.3)", background: "white" } : { border: "1px solid rgba(211,156,59,0.2)", background: "rgba(6,3,18,0.6)" }}>
            ...
            <p className="text-gold-300/60 text-xs">{correct}/{quiz.length} correct · Next review: {nextIntervalText} ({nextReviewDateStr})</p>
          </div>
          {quiz.map((q, i) => (
