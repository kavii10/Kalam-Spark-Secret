...
import {
  ...
  Plus, Volume2, SkipBack, SkipForward, Send, Pencil, Mic, Paperclip,
  Globe, Library, Languages, Clock, Trash2, Copy, Share2, VolumeX
} from 'lucide-react';
...
const downloadFile = async (filename: string, contentBlob: Blob, textContent?: string) => {
  if (Capacitor.isNativePlatform()) {
    try {
      ...
      if (status.publicStorage !== 'granted') {
        ...
        if (requestStatus.publicStorage !== 'granted') {
          throw new Error('Storage permission denied. Please enable storage permission in settings.');
        }
      }
      ...
      if (textContent !== undefined) {
        ...
        alert(`📄 Transcript saved to Documents/${filename}`);
      } else {
        ...
        alert(`✅ Audio saved to Documents/${filename}`);
      }
    } catch (e: any) {
      alert(`Save failed: ${e.message || e}`);
    }
  } else {
    const objectUrl = textContent !== undefined 
...
/* --- Lightweight TTS player for podcast library items (mobile + desktop fallback) --- */
function LibraryTTSPlayer({ lines, podcastLang, host1, host2, durationEst, user }: {
  ...
  const resumeTimerRef = React.useRef<ReturnType<typeof setInterval> | null>(null);
  ...
  const handleScrub = (clientX: number) => {
    if (!progressBarRef.current || !lines || lines.length === 0) return;
    const rect = progressBarRef.current.getBoundingClientRect();
    const ratio = Math.max(0, Math.min(1, (clientX - rect.left) / rect.width));
    const targetIdx = Math.max(0, Math.min(lines.length - 1, Math.floor(ratio * lines.length)));
  ...
  // Android Chrome pauses speechSynthesis after ~30 seconds — keep it alive
  ...
  const playFrom = (startIdx: number) => {
    ...
    const speakSingleLine = (i: number): Promise<void> => {
      return new Promise<void>((resolve) => {
        ...
        const estimatedMs = Math.max(5000, Math.ceil((wordCount / 85) * 60_000) + 4000);
        safetyTimer = setTimeout(() => {
          console.warn(`[LibraryTTSPlayer] Safety timeout on line ${i}, advancing.`);
          done();
        }, estimatedMs);

        if (llamaPlugin.isSupported()) {
          llamaPlugin.speak(line.text, lang, (status) => {
            if (status === 'done' || status === 'error') done();
          }).catch((err) => {
            console.warn('[LibraryTTSPlayer] Native speak failed, falling back to Web Speech:', err);
            speakViaWebSpeech();
          });
          return;
        }
        ...
        function speakViaWebSpeech() {
          ...
          // Workaround for stuck speech synthesis queue
          window.speechSynthesis.cancel();
          ...
          utt.onend = () => done();
          utt.onerror = (e) => {
            console.warn(`[LibraryTTSPlayer] TTS error on line ${i}:`, e);
            done();
          };
    ...
    const speakLine = async (i: number) => {
      if (playTokenRef.current !== currentToken || !activeRef.current) return;
      if (i >= lines.length) {
        setPlaying(false);
        setLineIdx(0);
        activeRef.current = false;
  ...
  return (
    <div className="space-y-2">
      <div 
        ref={progressBarRef}
        className="group relative w-full h-2 bg-zinc-700/50 rounded-full cursor-pointer mt-3"
        onMouseDown={(e) => {
      ...
      >
        ...
        />
        <div 
          className={`absolute top-1/2 -translate-y-1/2 w-3.5 h-3.5 bg-white rounded-full shadow-[0_0_10px_rgba(124,58,237,0.6)] border border-violet-500 z-10 transition-transform group-hover:scale-110 ${!isDragging ? 'transition-all duration-100' : ''}`} 
      ...
      <div className="flex items-center gap-2">
        <button 
          onClick={() => {
            const target = Math.max(0, lineIdx - 1);
            if (playing) {
              pause();
        ...
        <button 
          onClick={() => {
            const target = Math.min(lines.length - 1, lineIdx + 1);
            if (playing) {
              pause();
              setTimeout(() => playFrom(target), 50);
            } else {
...
function LibraryAudioPlayer({ audioFilename, lines, podcastLang, host1, host2, durationEst, user, onLoadError }: {
  ...
  const handleLoadAndPlay = async () => {
    if (blobUrl || loading || audioError) return;
    setLoading(true);
    try {
  ...
  useEffect(() => {
    // Auto-attempt loading as soon as we have a BACKEND URL
    if (BACKEND && audioFilename) {
      handleLoadAndPlay();
    } else {
      setAudioError(true);
    }
  ...
  // If audio fails (no backend, file missing, etc.) — show TTS player
  if (audioError || (!BACKEND && lines && lines.length > 0)) {
    return (
      <LibraryTTSPlayer
        lines={lines || []}
        podcastLang={podcastLang || 'en'}
        host1={host1 || 'Host 1'}
        host2={host2 || 'Host 2'}
...
function AudioPlayer({ src, host1, host2, linesCount, durationEst, downloadUrl, audioRef, user, lines, podcastLang, isLocal, controlRef }: { 
  src: string; 
  host1: string; 
  host2: string; 
  linesCount: number; 
  ...
  podcastLang?: string;
  isLocal?: boolean;
  controlRef?: React.MutableRefObject<{ pause: () => void; play: () => void; isPlaying: () => boolean } | null>;
}) {
  ...
  // Fallback to TTS only if explicitly local, backend is missing/offline, or backend playback failed
  ...
  const playTTS = (startIdx = 0) => {
    ...
    // Android Chrome pauses speechSynthesis after ~30s — keep it alive with periodic resume()
    if ('speechSynthesis' in window) {
    ...
    /**
     * Speak a single line. Returns a Promise that resolves when the line
     * has finished speaking (or after a safety timeout). This guarantees
     * the chain always advances even if the native 'speakStatus' event
     * is never fired (e.g. listener race or Android TTS delay).
     */
    const speakSingleLine = (i: number): Promise<void> => {
      return new Promise<void>((resolve) => {
        const line = lines[i];
        ...
        // Estimate speaking time: ~120 words/min, plus 500ms buffer
        const wordCount = line.text.split(/\s+/).length;
        const estimatedMs = Math.max(5000, Math.ceil((wordCount / 85) * 60_000) + 4000);
        safetyTimer = setTimeout(() => {
          console.warn(`[AudioPlayer TTS] Safety timeout on line ${i}, advancing.`);
          done();
        }, estimatedMs);

        if (llamaPlugin.isSupported()) {
          llamaPlugin.speak(line.text, targetLang, (status) => {
            if (status === 'done' || status === 'error') done();
          }, speed).catch((err) => {
            console.warn('[AudioPlayer TTS] Native speak failed, falling back to Web Speech:', err);
            speakViaWebSpeech();
          });
          return;
        }
        ...
        function speakViaWebSpeech() {
          ...
          utt.onend = () => done();
          utt.onerror = (e) => {
            console.warn(`[AudioPlayer TTS] Web Speech error on line ${i}:`, e);
            done();
          };
          window.speechSynthesis.speak(utt);
        }
    ...
    // Sequential speaker loop
    const speakLine = async (i: number) => {
      if (playTokenRef.current !== currentToken || !ttsActiveRef.current) return;
      if (i >= lines.length) {
        setPlaying(false);
        setTtsLineIdx(0);
      ...
      await speakSingleLine(i);
      if (playTokenRef.current !== currentToken) return;
      speakLine(i + 1); // advance (non-awaited tail call is intentional)
    };

    // Ensure Web Speech voices are loaded before starting
    setTimeout(() => {
      if (typeof window === 'undefined' || !('speechSynthesis' in window)) {
        speakLine(startIdx);
        return;
  ...
  }, [controlRef, useTTS, playing, ttsLineIdx, audioRef]);
  ...
  const handleScrub = (clientX: number) => {
    ...
    if (useTTS) {
      if (!lines || lines.length === 0) return;
      const targetIdx = Math.max(0, Math.min(lines.length - 1, Math.floor(ratio * lines.length)));
      setTtsLineIdx(targetIdx); // immediate UI feedback
    } else {
  ...
  }, [isDragging, current, ttsLineIdx, playing, useTTS, audioRef]);
  ...
  if (useTTS) {
    ...
    const handleTTSDownload = async () => {
      // Path 1: If a server audio file URL is available and accessible, download it as MP3
      const hasValidDownloadUrl = downloadUrl && !downloadUrl.endsWith('/undefined')
        && !downloadUrl.endsWith('/null') && !downloadUrl.endsWith('/') && !downloadUrl.endsWith('//');
      
      if (hasValidDownloadUrl && BACKEND) {
        try {
          const filename = `podcast_${linesCount}_lines.mp3`;
          const res = await fetch(downloadUrl);
          ...
          console.warn("[AudioPlayer TTS] Server audio download failed, will try transcript:", e);
        }
      }

      // Path 2: No backend audio available (offline/local podcast).
      // Save the transcript as a plain text file with clear labelling.
      if (!lines || lines.length === 0) return;
      const transcript = [
        `=== PODCAST TRANSCRIPT ===`,
        `Lines: ${linesCount}`,
        `Note: Audio is only available when connected to the backend server.`,
        ``,
        ...lines.map(l => `${l.speaker}:\n${l.text}`)
      ].join('\n\n');
      const filename = `podcast_transcript_${linesCount}_lines.txt`;
      await downloadFile(filename, new Blob(), transcript);
    };
      ...
      <div className="space-y-3">
        ...
        <div className="flex items-center gap-3">
          <button
            onClick={() => {
              if (!lines || lines.length === 0) return;
              const targetIdx = Math.max(0, ttsLineIdx - 1);
              if (playing) {
                pauseTTS();
                setTimeout(() => playTTS(targetIdx), 50);
              } else {
                setTtsLineIdx(targetIdx);
              }
            }}
            title="Skip backward 1 line"
          ...
          <button
            onClick={() => {
              if (!lines || lines.length === 0) return;
              const targetIdx = Math.min(lines.length - 1, ttsLineIdx + 1);
              if (playing) {
                pauseTTS();
                setTimeout(() => playTTS(targetIdx), 50);
              } else {
          ...
          <button
            ...
            className="p-2 rounded-lg bg-zinc-800 hover:bg-zinc-700 text-zinc-400 hover:text-violet-400 transition-all"
          >
            <Download size={16} />
          </button>
        </div>
  ...
  return (
    <div className="space-y-3">
      <audio 
        ...
        preload="metadata" 
        onError={() => {
          console.warn("[AudioPlayer] HTML audio load failed, falling back to TTS");
          setAudioFailed(true);
        }}
      />
...
export default function FileSpeaker({ user, setUser, isLight }: { user: UserProfile; setUser: (u: UserProfile) => void; isLight: boolean }) {
  ...
  const [interactSpeakingIdx, setInteractSpeakingIdx] = useState<number | null>(null);
  const podcastControlRef = useRef<{ pause: () => void; play: () => void; isPlaying: () => boolean } | null>(null);
  ...
  const [podcastLibrary, setPodcastLibrary] = useState<PodcastRecord[]>(() => {
    ...
    try {
      const key = 'fs_podcast_library_' + (user.id || user.email || 'guest');
  ...
  useEffect(() => {
    ...
    } else if (!user.podcasts) {
      try {
        const key = 'fs_podcast_library_' + (user.id || user.email || 'guest');
        setPodcastLibrary(JSON.parse(localStorage.getItem(key) || '[]'));
      } catch {
        setPodcastLibrary([]);
      }
    }
  ...
  const savePodcastsToUser = (newLib: PodcastRecord[]) => {
    ...
    const key = 'fs_podcast_library_' + (user.id || user.email || 'guest');
    localStorage.setItem(key, JSON.stringify(newLib));
  };

  // Stop Q&A interaction speech when page changes/unmounts
  useEffect(() => {
    return () => {
      if (llamaPlugin.isSupported()) {
        llamaPlugin.stopSpeak();
  ...
  const handleInteractSpeak = (i: number, text: string) => {
    if (interactSpeakingIdx === i) {
      if (llamaPlugin.isSupported()) {
        llamaPlugin.stopSpeak();
      }
      if (typeof window !== 'undefined' && window.speechSynthesis) {
    ...
    setInteractSpeakingIdx(i);

    const done = () => {
      setInteractSpeakingIdx(prev => prev === i ? null : prev);
    };
    ...
    if (llamaPlugin.isSupported()) {
      ...
      llamaPlugin.speak(text, targetLang, (status) => {
        if (status === 'done' || status === 'error') done();
      }).catch((err) => {
        console.warn('[QA Speech] Native speak failed, falling back to Web Speech:', err);
        speakViaWebSpeech();
      });
      return;
    }
  ...
  const downloadPodcastFromLibrary = async (recId: string, filename: string, audioFilename: string, rec?: PodcastRecord) => {
    if (downloadingIds[recId]) return;
    setDownloadingIds(prev => ({ ...prev, [recId]: true }));
    try {
      // Save transcript only if no backend is configured or no filename is provided
      if (!BACKEND || !audioFilename) {
        const podRec = rec || podcastLibrary.find(p => p.id === recId);
        if (podRec?.lines && podRec.lines.length > 0) {
          const transcript = [
            `=== PODCAST TRANSCRIPT ===`,
            `Topic: ${podRec.topic || podRec.sourceTitle}`,
            `Hosts: ${podRec.host1} & ${podRec.host2}`,
            `Lines: ${podRec.linesCount}`,
            ``,
            ...podRec.lines.map(l => `${l.speaker}:\n${l.text}`)
          ].join('\n\n');
          const txtFilename = filename.replace(/\.mp3$/, '.txt');
          await downloadFile(txtFilename, new Blob(), transcript);
        } else {
          alert('No transcript available for this podcast.');
        }
        return;
      }
      const url = `${BACKEND}/api/filespeaker/audio/${audioFilename}`;
  ...
  /* ─── Chat ─── */
  const handleChat = async (customQ?: string, customHistory?: ChatMsg[]) => {
    const q = (customQ !== undefined ? customQ : chatInput).trim();
    if (!q || !activeSource || chatLoading) return;
    if (customQ === undefined) {
      setChatInput('');
    }
    ...
    const historyForApi = customHistory !== undefined ? customHistory : prev.chat;
    setSourceStates(cur => ({
      ...cur,
      [sid]: { ...cur[sid], chat: [...historyForApi, userMsg] }
    }));
    setChatLoading(true);
...
Be accurate and concise. Never invent facts.`;
  ...
  const handleEditChatMsgSave = async (idx: number, newText: string) => {
    if (!newText.trim() || !activeSource) return;
    const sid = activeSource.source_id;
    const historyBefore = getState(sid).chat.slice(0, idx);
    setEditingMsgIdx(null);
    setEditingMsgText('');
    await handleChat(newText, historyBefore);
  };
  ...
  const handleReadAloud = async (idx: number, text: string) => {
    ...
    if (llamaPlugin.isSupported()) {
      try {
        await llamaPlugin.speak(cleanText, lang, (status) => {
        ...
        console.warn('[FileSpeaker] Native speak failed, trying Web Speech API...', err);
      }
    }
...
Each line: 1-3 natural sentences. Conversational and educational.`;
      ...
      // Stop any pre-existing TTS — the AudioPlayer's Play button controls playback from here.
      // (Do NOT queue all lines here; that conflicts with AudioPlayer's own TTS chain.)
      if (llamaPlugin.isSupported()) {
        llamaPlugin.stopSpeak();
  ...
  const handlePodcastInteract = async () => {
    ...
    const wasPodcastPlaying = podcastControlRef.current?.isPlaying() || false;
    podcastControlRef.current?.pause();
    ...
    try {
      ...
      const onSpeechEnd = () => {
        if (wasPodcastPlaying) {
          podcastControlRef.current?.play();
        }
      };
      ...
      if (replyAudioUrl === 'local_tts') {
        if (llamaPlugin.isSupported()) {
          ...
          const targetLang = langMap[podcastLang] || 'en-US';
          llamaPlugin.speak(replyText, targetLang, (status) => {
            if (status === 'done' || status === 'error') {
              onSpeechEnd();
            }
          }).catch((err) => {
        ...
        function speakViaWebSpeech() {
          ...
          ut.pitch = 0.9;
          ut.rate = 1.0;
          ut.onend = onSpeechEnd;
          ut.onerror = onSpeechEnd;
          window.speechSynthesis.speak(ut);
        }
      } else {
    ...
    <div className="fade-up h-full flex flex-col gap-6 bg-transparent p-5 lg:p-8 lg:pb-6 overflow-hidden">
      ...
      <div className="flex flex-col lg:flex-row gap-5 flex-1 min-h-0">
          ...
          <div className="flex flex-col gap-2 overflow-y-auto flex-1 pr-1 pb-4">
            ...
            {sources.map(src => (
              <div key={src.source_id}
                className={`rounded-xl border transition-all group relative flex items-center ${
                ...
                {/* Left side: checkbox (completely separate from the switchSource button) */}
                <div className="pl-3 py-3 flex items-center justify-center shrink-0">
                ...
                {/* Right side: Clickable card body to switch active source */}
                <button onClick={() => switchSource(src)} className="flex-1 text-left p-3 pl-2 pr-14 min-w-0">
        ...
        <div className={`flex-1 flex flex-col min-w-0 h-full overflow-hidden transition-all duration-300 ${
          activeSource 
            ? 'fixed inset-0 z-[60] bg-gradient-to-b from-[#020713] via-[#040d24] to-[#020713] sm:relative sm:z-auto sm:inset-auto sm:bg-transparent' 
            : 'hidden sm:flex'
        }`}>
          {!activeSource ? (
                ...
                <div className="flex-1 flex flex-col overflow-hidden">
                  <div className="flex-1 overflow-y-auto p-5 space-y-4">
                    ...
                    {chatHistory.map((msg, i) => (
                      <div key={i} className={`flex gap-3 group/msg ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                            ...
                            {editingMsgIdx === i ? (
                              <div className="flex flex-col gap-2 min-w-[200px]">
                                <textarea
                                  ...
                                  className={`w-full text-xs p-2 rounded border focus:outline-none focus:border-violet-500 ${
                                    isLight ? 'bg-white text-zinc-800 border-zinc-300' : 'bg-zinc-900 text-zinc-100 border-zinc-700'
                                  }`}
                                  rows={3}
                                />
                                <div className="flex justify-end gap-2">
                                  <button
                                    ...
                                    className="px-2 py-1 rounded text-[10px] border border-zinc-500/30 text-zinc-400 hover:text-zinc-200 hover:bg-zinc-800"
                                  >
                                    Cancel
                                  </button>
                                  <button
                                    onClick={() => handleEditChatMsgSave(i, editingMsgText)}
                                    className="px-2.5 py-1 rounded text-[10px] bg-violet-600 text-white hover:bg-violet-500"
                                  >
                                    Save
                                  </button>
                                </div>
                          ...
                          <div className={`flex items-center gap-1.5 mt-1 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                            ...
                            <button
                              ...
                              className={`flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] transition-all ${
                                isLight ? 'text-zinc-400 hover:text-zinc-700 hover:bg-zinc-100' : 'text-zinc-600 hover:text-zinc-300 hover:bg-zinc-800'
                              }`}
                            >
                              <Copy size={11} /> Copy
                            </button>
                            ...
                            <button
                              title="Read Aloud"
                              onClick={() => handleReadAloud(i, msg.text)}
                              className={`flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] transition-all ${
                                speakingIdx === i
                                  ? 'text-violet-400 bg-violet-500/10'
                                  : isLight ? 'text-zinc-400 hover:text-violet-600 hover:bg-violet-50' : 'text-zinc-600 hover:text-violet-400 hover:bg-violet-500/10'
                              }`}
                            >
                              {speakingIdx === i ? <VolumeX size={11} /> : <Volume2 size={11} />} {speakingIdx === i ? 'Mute' : 'Read'}
                            </button>
                            {/* Share (AI message only) */}
                            {msg.role === 'ai' && (
                              <button
                                title="Share"
                                onClick={() => {
                                ...
                                className={`flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] transition-all ${
                                  isLight ? 'text-zinc-400 hover:text-blue-600 hover:bg-blue-50' : 'text-zinc-600 hover:text-blue-400 hover:bg-blue-500/10'
                                }`}
                              >
                                <Share2 size={11} /> Share
                              </button>
                            )}
                            {/* Edit (User message only) */}
                            {msg.role === 'user' && (
                              <button
                                title="Edit"
                                onClick={() => {
                                ...
                                className={`flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] transition-all ${
                                  isLight ? 'text-zinc-400 hover:text-amber-600 hover:bg-amber-50' : 'text-zinc-600 hover:text-amber-400 hover:bg-amber-500/10'
                                }`}
                              >
                                <Pencil size={11} /> Edit
                              </button>
                            ...
                            <button
                              ...
                              onClick={() => {
                                ...
                                if (confirm('Delete this message?')) {
                                  setSourceStates(cur => {
                                    const s = cur[sid] ?? { chat: [], transforms: [], podcast: null };
                                    return { ...cur, [sid]: { ...s, chat: s.chat.filter((_, idx) => idx !== i) } };
                                  });
                                }
                              }}
                              className={`flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] transition-all ${
                                isLight ? 'text-zinc-400 hover:text-red-600 hover:bg-red-50' : 'text-zinc-600 hover:text-red-400 hover:bg-red-500/10'
                              }`}
                            >
                  ...
                  <div className={`p-3 border-t flex items-center gap-2 ${isLight ? 'border-zinc-200 bg-zinc-50' : 'border-zinc-800/60 bg-zinc-900/40'}`}>
                    ...
                    <button 
                      ...
                      className={`p-3 rounded-lg border transition-all shrink-0 ${
                        isLight 
                          ? 'bg-white border-zinc-200 text-zinc-500 hover:bg-zinc-50 disabled:opacity-50' 
                          : 'bg-zinc-800/50 border-zinc-700/50 text-zinc-400 hover:text-white hover:bg-zinc-800 disabled:opacity-50'
                      }`}
                      title="Attach file to sources"
                    >
                    ...
                    <button 
                      ...
                      className={`px-4 py-3 rounded-lg transition-all ${chatInput.trim() && !chatLoading ? 'bg-violet-600 text-white hover:bg-violet-500 cursor-pointer' : isLight ? 'bg-zinc-200 text-zinc-400 cursor-not-allowed' : 'bg-zinc-800 text-zinc-600 cursor-not-allowed'}`}>
                      <Send size={16} />
                    </button>
                  </div>
                </div>
                  ...
                  {podcast && (
                    <div className={`rounded-xl border p-5 space-y-4 ${isLight ? 'border-violet-300/40 bg-violet-50' : 'border-violet-500/30 bg-violet-500/5'}`}>
                      ...
                      <AudioPlayer
                        ...
                        user={user}
                        lines={podcast.lines || []}
                        podcastLang={podcast.language || podcastLang || 'en'}
                        isLocal={podcast.is_local}
                        controlRef={podcastControlRef}
                      />
                          ...
                          <div className="relative flex-1 w-full">
                            ...
                            <div className="absolute right-1.5 top-1/2 -translate-y-1/2 flex items-center gap-1">
                              ...
                              <button 
                                onPointerDown={(e) => {
                                  e.preventDefault();
                                  if (interactQ.trim() && !interactLoading) {
                                    handlePodcastInteract();
                                  }
                                }}
                                onClick={(e) => {
                                  e.preventDefault();
                                }}
                                disabled={!interactQ.trim() || interactLoading}
                            ...
                            {podcast.interactions.map((interaction: any, i: number) => (
                              <div key={i} className={`border rounded-xl p-3 ${isLight ? 'bg-white border-zinc-200 shadow-sm' : 'bg-zinc-900/60 border-zinc-800'}`}>
                                ...
                                <div className="flex items-start gap-3">
                                  {interaction.audio === 'local_tts' || !BACKEND || !networkService.isOnline() ? (
                                    <button
                                      onClick={() => handleInteractSpeak(i, interaction.a)}
                                      className={`p-1.5 rounded-lg flex items-center justify-center transition-all ${
                                        interactSpeakingIdx === i 
                                          ? 'bg-violet-500/20 text-violet-400 border border-violet-500/30' 
                                          : isLight ? 'bg-zinc-100 hover:bg-zinc-200 text-zinc-600' : 'bg-zinc-800 hover:bg-zinc-700 text-zinc-300'
                                      }`}
                                      title={interactSpeakingIdx === i ? "Stop speaking" : "Listen to response"}
                                    >
                                      {interactSpeakingIdx === i ? <VolumeX size={14} /> : <Volume2 size={14} />}
                                    </button>
                                  ) : (
              ...
              {podcastLibrary.length === 0 ? (
                ...
                podcastLibrary.map(rec => (
                  <div key={rec.id} className={`rounded-xl border p-4 ${
                    ...
                    <div className="mt-3">
                      {/* Unified player: LibraryAudioPlayer auto-falls back to TTS when audio is unavailable */}
                      <div className="space-y-2">
                        <LibraryAudioPlayer
                        ...
                        <button
                          onClick={() => downloadPodcastFromLibrary(rec.id, `podcast_${rec.id}.mp3`, rec.audioFilename, rec)}
                          disabled={downloadingIds[rec.id]}
                          title={BACKEND && rec.audioFilename ? 'Download podcast audio' : 'Save transcript'}
                          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[11px] font-medium border transition-all disabled:opacity-50 cursor-pointer ${
                          ...
                          {downloadingIds[rec.id] ? (
                            <><Loader2 size={11} className="animate-spin text-violet-400" /> Saving...</>
                          ) : (
                            <><Download size={11} /> {BACKEND && rec.audioFilename ? 'Download MP3' : 'Save Transcript'}</>
                          )}
                        </button>
                      </div>
                    </div>
