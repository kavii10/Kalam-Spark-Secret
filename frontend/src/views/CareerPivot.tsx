...
export default function CareerPivot({ user, setUser }: Props) {
  ...
  const handleAnalyze = async () => {
    ...
    if (isIllegalCareer(newDream.trim())) {
      setError("Kalam Spark does not support or generate roadmaps for illegal, harmful, or dangerous activities. Please choose a legal, constructive career path to explore.");
      return;
    }
    setLoading(true);
    setResult(null);
