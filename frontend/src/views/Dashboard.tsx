...
import { grantReward, makeStreakReward } from '../services/rewardService';
...
export default function Dashboard({ user, setUser }: { user: UserProfile; setUser?: React.Dispatch<React.SetStateAction<UserProfile>> }) {
  ...
  useEffect(() => {
    if (user.streak >= 7 && !user.rewards?.some(r => r.id === 'streak_7') && setUser) {
      const reward = makeStreakReward(7);
      grantReward(user, reward, setUser);
    }
  }, [user.streak, user.rewards, setUser]);
          ...
          <div className="glass-card p-6 md:col-span-3 space-y-6" style={{ borderColor: 'rgba(255,140,66,0.22)' }}>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-6">
              <div>
                ...
                {/* 7-Day Streak Reward Tracker */}
                <div className="mt-4 p-4 rounded-2xl border border-orange-500/25 bg-orange-500/5 max-w-md">
                  <div className="flex justify-between items-center text-xs font-bold mb-1.5">
                    <span className="text-gold-400">Weekly Reward Progress</span>
                    <span className="text-orange-400 bg-orange-500/10 px-2 py-0.5 rounded-full text-[10px]">
                      {Math.min(user.streak || 0, 7)} / 7 Days
                    </span>
                  </div>
                  <div className="w-full bg-zinc-800/80 h-2.5 rounded-full overflow-hidden border border-white/5 relative">
                    <div 
                      className="bg-gradient-to-r from-orange-500 via-amber-500 to-yellow-400 h-full rounded-full transition-all duration-700" 
                      style={{ width: `${Math.min(((user.streak || 0) / 7) * 100, 100)}%` }} 
                    />
                  </div>
                  <div className="text-[10px] mt-2 flex items-center gap-1.5 leading-normal">
                    {user.streak >= 7 ? (
                      <span className="text-emerald-400 flex items-center gap-1">
                        🎉 <strong>Weekly Milestone Met!</strong> You earned the 7-Day Streak Reward (+150 XP)!
                      </span>
                    ) : (
                      <span className={`${isLight ? 'text-zinc-500' : 'text-gold-200/50'}`}>
                        🔥 Keep it up! <strong>{7 - (user.streak || 0)} more days</strong> to claim the 7-Day Streak Reward (+150 XP).
                      </span>
                    )}
                  </div>
                </div>
