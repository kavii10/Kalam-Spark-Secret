...
import { checkAndGenerateDailyTasks } from '../services/taskBackgroundService';
...
async function generateRoadmapWithProgress(user: UserProfile, setLoadingMsg: (msg: string) => void): Promise<CareerRoadmap> {
  ...
  // Start the actual roadmap generation promise in the background
  const roadmapPromise = generateRoadmap(user);

  // Show progress messages in a paced, sequential order
  for (let i = 0; i < steps.length; i++) {
    setLoadingMsg(steps[i]);
    // Show each message for at least 1200ms
    await new Promise(resolve => setTimeout(resolve, 1200));
  }

  setLoadingMsg("⠋ Finalizing blueprint details...");
  try {
...
export default function RoadmapView({
  ...
  // Derived state directly from user profile settings (single source of truth)
  // @ts-ignore
  const conceptProgress = user.settings?.conceptProgress || {};
  // @ts-ignore
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (
        ...
        lastLoadedRef.current.userId === currentKey.userId &&
        lastLoadedRef.current.dream === currentKey.dream &&
        lastLoadedRef.current.branch === currentKey.branch &&
        lastLoadedRef.current.year === currentKey.year &&
        !forceRefresh
      ) {
        return;
      }
      ...
      try {
        ...
        setUser(clearedUser);
        await dbService.saveUser(clearedUser);
        ...
        try {
          ...
          await dbService.saveRoadmap(user, clean);
          checkAndGenerateDailyTasks(user, setUser, true).catch(err => {
            console.error('[RoadmapView] Background task generation failed:', err);
          });
          setLoading(false);
  ...
  const toggleConcept = (stageId: string, stageIndex: number, concept: string, totalConcepts: number) => {
    ...
    const currentConcepts = conceptProgress[stageId] || [];
    let nextConcepts: string[];
    if (currentConcepts.includes(concept)) {
      nextConcepts = currentConcepts.filter(c => c !== concept);
    } else {
      nextConcepts = [...currentConcepts, concept];
    }

    const nextProgress = {
      ...conceptProgress,
      [stageId]: nextConcepts
    };
    ...
    const updatedUser = {
      ...
      settings: {
        ...
        notificationsEnabled: user.settings?.notificationsEnabled ?? true,
        soundEnabled: user.settings?.soundEnabled ?? true,
        conceptProgress: nextProgress
      }
    };

    localStorage.setItem('kalamspark_concept_progress', JSON.stringify(nextProgress));
    setUser(updatedUser);
    dbService.saveUser(updatedUser);

    checkStageCompletion(stageId, stageIndex, nextConcepts, undefined);
  };
  ...
  const toggleProject = (stageId: string, stageIndex: number, project: string) => {
    ...
    const currentProjects = projectProgress[stageId] || [];
    let nextProjects: string[];
    if (currentProjects.includes(project)) {
      nextProjects = currentProjects.filter(p => p !== project);
    } else {
      nextProjects = [...currentProjects, project];
    }

    const nextProgress = {
      ...projectProgress,
      [stageId]: nextProjects
    };
    ...
    localStorage.setItem('kalamspark_project_progress', JSON.stringify(nextProgress));
    setUser(updatedUser);
    dbService.saveUser(updatedUser);

    checkStageCompletion(stageId, stageIndex, undefined, nextProjects);
  };
  ...
  const handleMarkIncomplete = async (stageId: string, stageIndex: number) => {
    try {
      ...
      const nextConcepts = { ...conceptProgress };
      delete nextConcepts[stageId];
      localStorage.setItem('kalamspark_concept_progress', JSON.stringify(nextConcepts));

      const nextProjects = { ...projectProgress };
      delete nextProjects[stageId];
      localStorage.setItem('kalamspark_project_progress', JSON.stringify(nextProjects));
