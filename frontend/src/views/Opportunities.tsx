...
export default function Opportunities({ user }: Props) {
  ...
  const handleFetchJobs = async () => {
    if (!networkService.isOnline()) {
      setJobsError("📡 Internet connection required to refresh jobs.");
      const cached = localStorage.getItem(`kalamspark_cached_jobs_${user.id}`);
      if (cached) {
        try {
    ...
    try {
      ...
      localStorage.setItem(`kalamspark_cached_jobs_${user.id}`, JSON.stringify(filteredJobs));
    } catch (err: any) {
  ...
  const handleFetchHackathons = async () => {
    if (!networkService.isOnline()) {
      setHackathonsError("📡 Internet connection required to refresh competitions.");
      const cached = localStorage.getItem(`kalamspark_cached_hackathons_${user.id}`);
      if (cached) {
        try {
          setHackathons(JSON.parse(cached));
        } catch (e) {
          console.error("Failed to parse cached hackathons:", e);
        }
      }
    ...
    try {
      ...
      if (!apiSucceeded) {
        if (!hasKeys) {
          warning = "🔌 Note: HackerEarth API credentials was missing or returned 0 items. Displaying global public online challenges.";
        }
        try {
      ...
      try {
        ...
        if (isApifyConfigured) {
          ...
          const unstopWarning = "🔌 Note: `APIFY_API_TOKEN` is missing or empty. Serving primary Devpost + HackerEarth challenges.";
      ...
      setHackathons(list);
      localStorage.setItem(`kalamspark_cached_hackathons_${user.id}`, JSON.stringify(list));
      if (warning) {
      ...
      <div className={`glass-card p-6 md:p-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 border-b ${
        ...
        <div className="space-y-2">
          <h1 className={`text-2xl md:text-3xl font-cinzel font-bold flex items-center gap-3 ${
            ...
            <Search size={28} /> Jobs & Competitions
          </h1>
          <p className={`text-xs md:text-sm max-w-xl ${isLight ? 'text-zinc-650' : 'text-zinc-400'}`}>
            Real-time feed sourcing open internships, competitions, and jobs pre-filtered for your profile from Adzuna, Devpost, HackerEarth, and Unstop.
          </p>
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between border-b border-zinc-800/30 pb-2 pt-2">
            ...
            <span className={`text-[10px] font-bold px-2.5 py-0.5 rounded-full border ${
              ...
              🔥 {filteredHackathons.length} Matches
            </span>
          </div>
          ...
          {hackathonsLoading ? (
            ...
            <div className={`border rounded-2xl p-16 text-center space-y-3 ${
              ...
              <h3 className="font-bold text-xs">No Active Competitions</h3>
              <p className="text-xs text-zinc-500 max-w-xs mx-auto">
