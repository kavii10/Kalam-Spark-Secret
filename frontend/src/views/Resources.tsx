...
import {
  ...
  ExternalLink, Loader2, ChevronLeft, ChevronRight, AlertCircle, WifiOff,
...
export default function Resources({ user }: { user: UserProfile }) {
    ...
    try {
      ...
      if (!isOnline) {
        if (cached && cached.books && cached.books.length > 0) {
          setOfflineWarning("📡 Offline mode — showing previously loaded resources.");
        } else {
  ...
  useEffect(() => {
    ...
    const handleOnline = () => {
      if (active) {
        console.log('[Resources] Went online, reloading resources...');
        fetchCurriculum();
      }
    };
  ...
  }, [fetchCurriculum, initialized]);

  // ── Load next batch — REPLACES current data with fresh resources ─────────────
  const loadNextBatch = useCallback(async () => {
    if (!networkService.isOnline()) {
      alert("📡 Internet connection required to load new resources.");
      return;
    }
  ...
  const handleSearch = async () => {
    if (!networkService.isOnline()) {
      alert("📡 Internet connection required to search study resources.");
      return;
    }
  ...
  return (
    <div className="space-y-6 fade-up">
      {offlineWarning && (
        <div className="flex items-center gap-2 p-3.5 rounded-xl text-xs font-semibold text-amber-400 bg-amber-950/40 border border-amber-500/20 animate-pulse">
                            ...
                            <div className="p-3 flex-1 flex flex-col justify-between min-w-0">
                              <div className="flex items-start justify-between gap-1.5 min-w-0">
                                ...
                                <button
                                  ...
                                  className="p-1.5 rounded-lg bg-red-500/10 hover:bg-red-500/20 text-red-400 border border-red-500/20 transition-all shadow-md shrink-0"
