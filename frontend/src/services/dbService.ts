...
function mapRow(data: any): UserProfile {
  ...
  return {
    ...
    lastTaskResetDate: data.last_task_reset_date || data.lastTaskResetDate || '',
    lastLoginDate: data.last_login_date || data.lastLoginDate || '',
...
function userToPayload(user: UserProfile): Record<string, any> {
  return {
    ...
    last_task_reset_date: user.lastTaskResetDate || '',
    last_sync: nowISO(),
