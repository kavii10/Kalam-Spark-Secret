...
class OfflineSyncService {
    ...
    for (const op of queue) {
      try {
        ...
        if (op.payload && op.payload.id) {
          ...
          if (op.type === 'save_task') storeName = 'tasks';
          else if (op.type === 'save_flashcard') storeName = 'flashcards';
          else if (op.type === 'save_flashcard_stats') storeName = 'flashcard_stats';
          else if (op.type === 'save_task_revision') storeName = 'task_revisions';
          else if (op.type === 'save_user') storeName = 'user_profile';
          
          if (storeName) {
            await localDB.markSynced(storeName, op.payload.id);
          }
        }
      } catch (err: any) {
  ...
  private async _executeOp(op: SyncOperation): Promise<void> {
    ...
    // Strip frontend-only metadata properties starting with '_' before passing to Supabase queries
    ...
    switch (type) {
      case 'save_user': {
        ...
        // Fallback minimal save (includes all columns to prevent data loss)
        const minPayload = {
          ...
          last_login_date: dbPayload.last_login_date,
          last_task_reset_date: dbPayload.last_task_reset_date,
          education_level: dbPayload.education_level || '',
          school_board: dbPayload.school_board || '',
          grade_or_semester: dbPayload.grade_or_semester || '',
          college_name: dbPayload.college_name || '',
          study_hours_per_day: dbPayload.study_hours_per_day ?? null,
          target_year: dbPayload.target_year || '',
          city: dbPayload.city || '',
          motivation: dbPayload.motivation || '',
          rewards: dbPayload.rewards ?? [],
          settings: dbPayload.settings || null,
          file_speaker_data: dbPayload.file_speaker_data || null,
          podcasts: dbPayload.podcasts ?? [],
        };
