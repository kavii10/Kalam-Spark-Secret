...
export interface LlamaPluginInterface {
  ...
  speak(options: { text: string; lang: string; utteranceId?: string; rate?: number }): Promise<void>;
...
let activeOnToken: ((token: string) => void) | null = null;
let activeCompletionListener: any = null;
let speakStatusCallback: ((status: 'start' | 'done' | 'error') => void) | null = null;

if (Capacitor.isNativePlatform()) {
  try {
    (LlamaPluginNative as any).addListener('speakStatus', (data: any) => {
      if (data && data.status && speakStatusCallback) {
        const currentStatus = data.status;
        setTimeout(() => {
          if (speakStatusCallback) {
            speakStatusCallback(currentStatus);
          }
        }, 0);
      }
    });
  } catch (e) {
    console.warn('[LlamaPlugin] Failed to register global speak status listener:', e);
  }
}
...
export const llamaPlugin = {
  ...
  async getCompletionStream(
    ...
    if (!activeCompletionListener) {
      try {
        activeCompletionListener = await (LlamaPluginNative as any).addListener('generationProgress', (data: any) => {
          if (data && data.token !== undefined && activeOnToken) {
            activeOnToken(data.token);
          }
        });
      } catch (err) {
        console.warn('[LlamaPlugin] Failed to register generation progress listener:', err);
      }
    }
  ...
  async stopCompletion() {
    ...
    if (this.isSupported()) {
      try {
        await LlamaPluginNative.stopCompletion();
      } catch (e) {
        console.warn('[LlamaPlugin] Failed to call native stopCompletion:', e);
      }
    }
  },
  ...
  async speak(text: string, lang: string, onStatus?: (status: 'start' | 'done' | 'error') => void, rate?: number): Promise<void> {
    if (!this.isSupported()) return;
    
    speakStatusCallback = onStatus || null;

    try {
      await LlamaPluginNative.speak({ text, lang, rate });
    } catch (err) {
      console.error('[LlamaPlugin] Native speak failed:', err);
      speakStatusCallback = null;
      throw err;
    }
  },
