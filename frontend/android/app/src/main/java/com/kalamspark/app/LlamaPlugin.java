...
@SuppressWarnings({"unused", "ResultOfMethodCallIgnored", "deprecation", "Convert2Lambda", "SdCardPath", "StringConcatenation", "RedundantSuppression", "Convert2TextBlock", "TextBlockMigration", "SpellCheckingInspection"})
...
public class LlamaPlugin extends Plugin {
    ...
    private volatile java.util.concurrent.CountDownLatch activeLatch = null;
    ...
    public void pickModelCallback(PluginCall call, ActivityResult result) {
        ...
        if (resultCode == Activity.RESULT_OK && result.getData() != null) {
            ...
            if (uri != null) {
                ...
                // Start copy AsyncTask directly passing the Uri
                Log.d(TAG, "pickModelCallback: Starting copy AsyncTask");
    ...
    private void copyModelFileAsyncTask(PluginCall call, final Uri uri) {
        // Resolve display name and file size on the callback thread
        ...
        Log.d(TAG, "copyModelFileAsyncTask: destination filename = " + finalDisplayName + ", size = " + finalTotalBytes);
                    ...
                    while ((bytesRead = is.read(buffer)) != -1) {
                        ...
                        if (now - lastNotificationTime > 300) { // throttle notifications
                            ...
                            if (finalTotalBytes > 0) {
                                progress = (int) ((bytesCopied * 100) / finalTotalBytes);
                            }
                             JSObject progressObj = createProgressObj(progress, bytesCopied, finalTotalBytes);
    ...
    private JSObject createProgressObj(int progress, long copied, long total) {
        JSObject progressObj = new JSObject();
        progressObj.put("status", "copying");
        progressObj.put("progress", progress);
        progressObj.put("copied", copied);
            ...
            public void run() {
                try {
                    ...
                    if (engine != null) {
                        ...
                        if (stopRequested) {
                            call.reject("Inference stopped by user");
                            return;
                        }
    ...
    public void speak(PluginCall call) {
        ...
        String rawLang = call.getString("lang", "en-US");
        final String lang = (rawLang != null) ? rawLang : "en-US";
        String rawUtteranceId = call.getString("utteranceId");
        final String utteranceId = (rawUtteranceId != null) ? rawUtteranceId : ("cap_tts_" + System.currentTimeMillis());
        Double rawRate = call.getDouble("rate");
        final float rate = (rawRate != null) ? rawRate.floatValue() : 1.0f;
