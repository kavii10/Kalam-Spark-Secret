...
# ──────────────────────────────────────────────
# Safe Career Filters (Illegal / Dangerous Careers Guard)
# ──────────────────────────────────────────────
def is_illegal_career(dream: str) -> bool:
    dream_lower = dream.lower().strip()
    
    # Comprehensive blacklist of illegal/harmful career terms
    illegal_keywords = [
    ...
    # Exceptions that represent legal/good/academic career paths
    legal_exceptions = [
    ...
    for kw in illegal_keywords:
        if kw in dream_lower:
            # Check if this query qualifies as a legal/academic/defense exception
            if any(exc in dream_lower for exc in legal_exceptions):
                return False
            return True
...
async def get_career_description(dream: str):
    ...
    if is_illegal_career(dream):
        raise HTTPException(
            status_code=400,
            detail="Kalam Spark does not support or provide descriptions for illegal, harmful, or dangerous activities. Please choose a legal, constructive career path to explore."
        )
    try:
        from llm_service import generate_career_description_llm
        description = await generate_career_description_llm(dream)
        return description
    except Exception as e:
        print(f"Error in get_career_description: {e}")
        raise HTTPException(status_code=500, detail=f"Failed to generate career description: {str(e)}")
...
async def get_roadmap(
    ...
    if is_illegal_career(dream):
        raise HTTPException(
            status_code=400,
            detail="Kalam Spark does not support or generate roadmaps for illegal, harmful, or dangerous activities. Please choose a legal, constructive career path to explore."
        )
    ...
    try:
        ...
        if is_illegal_career(dream):
            await websocket.send_json({
                "type": "error",
                "msg": "Kalam Spark does not support or generate roadmaps for illegal, harmful, or dangerous activities. Please choose a legal, constructive career path to explore."
            })
            return
